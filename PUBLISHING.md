...
Checklist and record for making this repo — and its GitHub wiki — public.
...
- [ ] **Account ID scrubbed** — no literal AWS account IDs in tracked files or the
      wiki. Use `<AWS_ACCOUNT_ID>`. (The wiki's `Cheatsheet-*` pages are already
      placeholdered.)
...
- [ ] **`.env` files** — only `.env.example` is tracked; real `.env.*` are gitignored
      ...
      the wiki's Cheatsheet-Git page).
...
- AWS account ID in `RUNBOOK.md` → **templated to `<AWS_ACCOUNT_ID>`** in the wiki's
  Runbook page. Keep it placeholdered in the wiki. The wiki's Cheatsheet-AWS page is
  likewise placeholdered (the original `lr-sandbox/aws_cheatsheet.txt` still carries
  the literal value — scrub if that file is ever published).
