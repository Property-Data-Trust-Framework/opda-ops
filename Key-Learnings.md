...
| `BYPASS_AUTH=true` silently breaks scope-protected endpoints | Bypass means the authorizer never runs, so `HttpContext.User` has no claims, so the scope filter 401s | Default `BYPASS_AUTH=false`. Only flip to `true` for very early bootstrap (before Raidiam certs); document and put back. |
